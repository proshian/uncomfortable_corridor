...
namespace Valve.VR.InteractionSystem
{
	...
	public class Player : MonoBehaviour
	{
		...
		protected virtual void Update()
		{
			...
			//Debug.Log(a_move.GetAxis(SteamVR_Input_Sources.RightHand));
			try
			{
				Camera playerCcamera = this.GetComponentInChildren<Camera>();
				CharacterController CharCntr = this.GetComponentInChildren<CharacterController>();

				Quaternion cameraRotation = playerCcamera.transform.rotation;

				Vector2 m = a_move.GetAxis(SteamVR_Input_Sources.LeftHand);
				//this.transform.position += Vector3.ProjectOnPlane(cameraRotation * new Vector3(m.x / 10, 0, m.y / 10)*speed, Vector3.up);
				CharCntr.Move(Vector3.ProjectOnPlane(cameraRotation * new Vector3(m.x / 10, 0, m.y / 10)*speed, Vector3.up));

			}
			catch (System.Exception e) { Debug.Log("trouble");}
